...
<div id="queue-{{ $queue->id }}" 
    ...
    <div class="pl-auto flex flex-row items-start self-start space-x-2">
        <!-- Supprimer -->
        <button title="Supprimer le post planifié" onclick="showQueueDeletePopup({{$queue->id}});"
    ...
    <!-- Queued post content -->
    <x-draft-content :content="$queue->content" />
        
    <!-- Tags -->
    @if (count($queue->tags) > 0)
    <div class="mb-3">
        <hr class="mb-3 border-gray-600" />
        @foreach ($queue->tags as $tag)
        <span class="p-1 m-1 rounded-md dark:bg-gray-900 dark:text-gray-400">
            #{{ $tag }}
        </span>
        @endforeach
    </div>
    ...
    <div class="dark:text-gray-400">
        <hr class="mb-3 border-gray-600" />
        Planifié pour {{ strftime('%d %B %Y à %H:%M', strtotime($queue->scheduled_time)) }}
    </div>
</div>
<div id="post-{{ $post->id }}" onclick="window.location.href = '/post/{{ $post->id }}'" {{ $attributes->merge(['class' => "cursor-pointer post bg-white hover:bg-white/50 dark:bg-gray-800 dark:hover:dark:bg-gray-700 overflow-hidden shadow-sm rounded-lg mb-4 md:p-5 p-2 md:mb-5 mb-3 w-full"]) }}>
    ...
    <div class="flex justify-between items-center">
        <div class="post-actions mt-4 flex items-center">
            ...
            <button title="Commenter" class="repost-button flex items-center text-gray-600 dark:text-gray-400 hover:text-blue-400 dark:hover:text-blue-500 mr-4">
                ...
                <span class="ml-1">{{ $post->original_shares->whereNotNull('content')->where('content', '!=', [])->count() }}</span>
            </button>
...
<div id="deleteConfirmationModal"
    ...
    <div
        ...
        <form action="{{ route('posts.deletePost', $post->id) }}" method="POST" style="display: inline;">
            @csrf
            @method('DELETE')
            <span class="text-xl flex flex-row pb-2 text-black dark:text-white">Êtes-vous sûr de vouloir supprimer ce post?</span>
            <span class="text-l flex flex-row pb-2 text-black dark:text-white">Celui-ci sera supprimé définitivement.</span>

<div {{ $attributes->merge(['class' =>"post-header flex items-center"]) }}>
    ...
    <div class="ml-auto flex flex-row items-start self-start space-x-2">
        ...
        @endif
        <!-- Supprimer -->
        <form action="{{ route('posts.deletePost', $postId) }}" method="POST" onsubmit="return confirm('Êtes-vous sûr de vouloir supprimer ce post? Celui-ci sera supprimé définitivement.');" style="display: inline;">
            @csrf
            @method('DELETE')

...
new class extends Component
{
    ...
    public function loadFollowedUsers()
    {
        ...
        $this->moreAvailable = $this->followedUsers->count() == $this->perPage;
    }
    ...
    public function loadMore()
    {
        ...
        $this->followedUsers = $this->followedUsers->merge($additionalUsers);
            ...
            <div class="post bg-white dark:bg-gray-800 overflow-hidden shadow-sm rounded-lg mb-4 md:p-5 p-2 md:mb-5 mb-3 w-full">
                <div class="flex items-center justify-between">
                    ...
                    <div class="flex items-center">
                        <!-- Image de profil -->
                        <img src="{{ $followedUser->getAvatar() }}" alt="Profile Image"
                        ...
                        <div>
                            ...
                            <p class="text-black dark:text-gray-100">Abonnés : {{ $followedUser->followers()->count() }}</p>
                            <p class="text-black dark:text-gray-100">Abonnements : {{ $followedUser->followed_users()->count() }}</p>
                        </div>
                    </div>
                    <div class="sm:mt-1 mt-5 sm:!ml-auto !ml-0 items-start self-start dark:!text-gray-100 transition duration-300 ease-in-out">
                        ...
                        <div id="dropdownMenu" class="hidden absolute top-32 right-72 bg-gray-700 dark:bg-gray-900 rounded-md shadow-lg z-50 max-h-48 overflow-auto">
                            <ul class="py-1 text-sm min-h-fit text-gray-200">
                                <li>
                                    <a href="{{ url('messages/user/' . $followedUser->id) }}" title="Envoyer un message à {{$followedUser->name}} ?" class="flex px-4 py-2 hover:bg-gray-600 items-center">
                                        <span class="mr-2">
                                ...
                                <li>
                                    <a href="" title="Bloquer {{$followedUser->name}} ?" class="flex px-4 py-2 hover:bg-gray-600 items-center">
                                        <span class="mr-2">
                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
                ...
                <div class="mt-4">
                    <p class="text-gray-600 dark:text-gray-300">{{ $followedUser->bio}}</p>
                    @if($followedUser->moderator)
